...
class MeshLib:
    ...
    def take_parallel_mesh_chain(cls, coarse, fine, bc_conditions):
        coarse = cls.stuff(coarse, bc_conditions)
        fine = cls.stuff(fine, bc_conditions)
        return cls(coarse), cls(fine)

    @classmethod
    def stuff(cls, B, bc_conditions):
        # # edges flipping
        ...
        mark_vertices,mark_edges, edges_number = cls.mark_neumann(B, bc_conditions)
        B['vertex_markers'] = mark_vertices
        B['edge_markers'] = mark_edges
        B['edges_number'] = edges_number
    ...
    #1 dirichlet 
        ...
        cls.vertex_number = np.zeros_like(mark_vertices)


        for v in range(len(mesh['vertex_markers'])):

            mark_vertices[v]=cls.check(mesh['vertices'][v],bc_conditions,v)


        edges_number = np.zeros_like(mark_edges)
        for v in range(len(mesh['edges'])):
            ...
            if cls.vertex_number[mesh['edges'][v,1]] != 0:
                edges_number[v] = cls.vertex_number[mesh['edges'][v,1]]
            elif cls.vertex_number[mesh['edges'][v,0]] != 0:
                edges_number[v] = cls.vertex_number[mesh['edges'][v,0]]
        ...
        return mark_vertices,mark_edges, edges_number

    @classmethod
    def check(cls,vertex,bc_conditions, v):
