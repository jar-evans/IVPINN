...
class VPINN():
    ...
    def custom_loss_IVPINN(self):
        ...
        for big_traingle in range(n_big_triangles):
            ...
            B_i, c_i, det_i, B_D_i, B_DD_i=self.B.change_of_coordinates(vertices_big)

            x_to_eval=B_i@self.B.Nodes.T +c_i


            x_to_eval=tf.reshape(x_to_eval,(-1,2))
            ...
            for index,triangle in enumerate(self.mesh['triangles'][little_triangles]):
                ...
                grad_elem_x=self.B.interpolate_dx_tf(xy_quad_total[little_triangles[index]],eval)  
                grad_elem_y=self.B.interpolate_dy_tf(xy_quad_total[little_triangles[index]],eval)  

                grad_elem=tf.concat([grad_elem_x,grad_elem_y],axis=1)

                grad_elem=tf.reshape(grad_elem,(2,-1))
