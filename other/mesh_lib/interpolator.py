...
# TODO clear structure so you can have up to order 2
...
class interpolator:
    def __init__(self, r: int, verbose: bool, pre: bool, points):
        ...
        self.M, self.M_dx, self.M_dy, self.M_d2x, self.M_d2y = self.generate_matrices(
            verbose
        )
        ...
        if pre == False:
            ...
            if points is None:
                raise ValueError(
                    "'you didnt provide points(np.array) for the pre_eval!. The script will terminate."
                )
            else:
    ...
    def print_polynomial(self, sol, pairs, index) -> None:
        ...
        for ii, (i, j) in enumerate(pairs):
            ...
            print(
                "\033[91m" + "\033[4m" + str(sol[ii]) + "\033[0m" + "\033[0m",
                s1,
                s2,
                end=" ",
            )
    ...
    def generate_matrices(self, verbose: bool):
        ...
        coeffs_dx = np.reshape(
            np.array(coeffs_dx, dtype=np_type), (self.n, len(self.powers_dy))
        )
        coeffs_dy = np.reshape(
            np.array(coeffs_dy, dtype=np_type), (self.n, len(self.powers_dy))
        )

        coeffs_d2x = np.reshape(
            np.array(coeffs_d2x, dtype=np_type), (self.n, len(self.powers_d2x))
        )
        coeffs_d2y = np.reshape(
            np.array(coeffs_d2y, dtype=np_type), (self.n, len(self.powers_d2y))
        )
    ...
    def eval_powers_a_priori(self, points) -> None:
        ...
        self.d2x_pre = np.ones(
            (np.shape(points)[0], len(self.powers_d2x)), dtype=np_type
        )
        self.d2y_pre = np.ones(
            (np.shape(points)[0], len(self.powers_d2y)), dtype=np_type
        )
