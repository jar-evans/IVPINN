...
class interpolator:
    ...
    def __init__(self, r: int,verbose:bool,pre:bool,points):
        """
        input=degree,verbose (True or False to show the polynomial), pre (set to True if the points are fixed), points (if pre is True, provide also the points)

        """

        # Here the cases are distinguished for r = 1, 2, etc.
        self.r = r
        ...
        # generate power pairs for the polynomial (x, y)
        ...
        # set the nodes for the interpolation in the reference triangle
        ...
        # generate the quadrature points for boundary integration
        self.generate_gaussian_quadrature(N=15) # integer represents order of accuracy

        # create matrices that represent the polynomials
        self.M ,self.M_dx,self.M_dy,self.M_d2x,self.M_d2y= self.generate_matrices(verbose)

        if verbose:
            self.plot_nodes()

        # correctly oredering the bases wrt to the nodes 
        # as follows: points on the nodes, and then points on the edges, and then points inside (anticlockwise)
        self.change_order()

        if verbose:
            self.plot_nodes()        

        # assigns preallocated points
        self.pre = pre
    ...
    def find_order(self, nodes):
        """
        Identifies the correct order of nodes, edges, int. points
        """
    ...
    def change_order(self):
        """
        Modifies order of nodes, edges and int. points to the correct order
        """
    ...
    def generate_powers_der(self, I: int):
        """
        generate  the correct powers for the derivatives and stores it 
        """
        der_x=[]
        der_y=[]
    ...
    def generate_interp_nodes(self, p: int):
        """
    ...
    def generate_coefficients(self, evaluation_matrix, verbose: bool):
        """
        Generates the results of Ax = bi
        """
        coeff = []
    ...
    def generate_matrices(self,verbose: bool):
        """
        Creates the matrices for interpolation as well as 1st/2nd order derivatives
        """
    ...
    def generate_M_der(self, M: Matrix, verbose:bool, I:int):
        """
        Generating the derivatives of the matrices
        """
        coeff_dx=[]
        coeff_dy=[]
    ...
    def eval(self,M,points,val,pairs): # used in training
        """
        generic function that interpolates a set of 2d nodes that are fixed in generic points inside the ref triangle,
        - M: the matrix with coeff of all the basis function in each row 
        - points: a np.array of size (n_points,2)
        - val: a np array of size (n_nodes,1)

        the output will be an col vector of size (n_points,1)
          
        """
 
        x=np.ones((np.shape(points)[0],len(pairs)),dtype=np_type)

        # can be optimised via tf.powers
        for ii,(i,j) in enumerate(pairs):
    ...
    def eval_pre(self,B,val):
        """
        Provides evaluation of all bases columnwise
        """
        return B @ val   

    ### Use this method if you want the eval of points that you don't know a priori 
    ### wrappers around numpy evaluation for different interpolations
    def interpolate(self,points,val):  # used in training
    ...
    def interpolate_dx(self,points,val): # used in training
        return self.eval(self.M_dx,points,val,self.powers_dx)
    
    def interpolate_dy(self,points,val): # used in training
    ...
    def eval_tf(self, M, points, val, pairs): # used in training
        """
        (tf version of prev. eval method)
        generic function that interpolates a set of 2d nodes that are fixed in generic points inside the ref triangle,
    ...
    ### wrappers around tensorflow evaluation for different interpolations
    def interpolate_tf(self, points, val): # used in training
        """points where you want to intepolate, val are the values at the fixed nodes
        if you set pre=True pass and empty second arguments as points"""
        return self.eval_tf(self.M, points, val, self.powers)

    def interpolate_dx_tf(self, points, val): # used in training
        return self.eval_tf(self.M_dx, points, val, self.powers_dx)

    def interpolate_dy_tf(self, points, val): # used in training
    ...
    def eval_powers_a_priori(self,points)->None:
        """
        Evaluates the basis and its derivatives at known points on the reference triangle
        """
    ...
    def change_of_coordinates(self, vertices: list):# used in training
        """
    ...
    def generate_gaussian_quadrature(self,N):
        """
        Creates the quadrature pairs (points and weights) for boundary integration
        """
        n=(N+1)//2
    ...
    # def gauss_lobatto_nodes_weights(self,n):
    #     nodes, _ = roots_legendre(n-1)
    #     nodes = np.concatenate(([-1.0], nodes, [1.0]))
    #     weights = np.ones_like(nodes) * 2.0 / (n * (n - 1))
    #     weights[0] = 1.0 / (n - 1)
    #     weights[-1] = 1.0 / (n - 1)
