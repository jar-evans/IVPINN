...
class VPINN(tf.keras.Model):
    ...
    def calc_residuals(self):
        ...
        #sum_of_vectors_vertices = tf.Variable(tf.zeros((self.n_vertices,1),dtype=tf.float64))
        #sum_of_vectors_edges = tf.Variable(tf.zeros((self.n_edges,1),dtype=tf.float64))
        sum_of_vectors_vertices=tf.zeros_like(self.sum_of_vectors_vertices)
        sum_of_vectors_edges=tf.zeros_like(self.sum_of_vectors_edges)
    ...
    def residual_summary(self,residual_vertices,residual__edges):
    ...
    def generate_boundary_points(self,n):
        ...
        boundary_points = self.generate_rectangle_points(a, b, c, d, n)
        ...
        return tf.reshape(u_bound_exact,(-1,1)),boundary_points
