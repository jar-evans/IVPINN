...
class PROBDEF:
    ...
    def neumann(self, x, y):
        ...
        h = lambda x, y: -3.2*x*np.cos(3.2*x*(x - y))*np.cos(x + 4.3*y) - 4.3*np.sin(3.2*x*(x - y))*np.sin(x + 4.3*y) + 2.6*np.sin(4.6*x + 9.2*y)*np.sin(5.2*x - 2.6*y) + 9.2*np.cos(4.6*x + 9.2*y)*np.cos(5.2*x - 2.6*y)
        return -1*f(x)*self.mu(x,0)*(1-y) + (y)*self.mu(x,1)*g(x)
        # return - f(x)*(1-y) + (y)*g(x)
        # return - h(x, 0)*self.mu(x, 0)*(1-y) + (y)*self.mu(x,1)*h(x,y)
